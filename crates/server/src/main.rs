...
/// What a FRESH room is built from: the circuit, its labels, and where the
/// hoist stands in it.
struct WorldDef {
    elements: Vec<ElementSpec>,
    panels: Vec<Panel>,
...
/// The sample world a fresh room boots into, chosen by `EE_WORLD` in the
/// same style as `EE_SAVE` / `EE_ADDR` / `EE_DIST`.
///
/// * unset (or anything else) — the showcase room, exactly as before.
/// * `synth` — the analog synthesizer in `synth.rs`.
///
/// This is only consulted when `load_room()` finds no save file, so it
/// composes with `EE_SAVE`: run the synthesizer with
/// `EE_SAVE=/tmp/synth.json EE_WORLD=synth`, and the showcase room's own
/// save is never touched.
fn world() -> WorldDef {
    match std::env::var("EE_WORLD").unwrap_or_default().as_str() {
        "synth" => WorldDef {
            elements: synth::synth_room_circuit(),
            panels: synth::synth_panels()
                .into_iter()
                .enumerate()
                .map(|(i, p)| Panel {
                    plid: i as u32 + 1,
...
async fn main() {
    ...
    let (mut elements, probes, next_pid, panels, next_plid, hoist, hoist_rect, damage) = match saved
    {
        ...
        None => {
            let w = world();
            let next_plid = w.panels.len() as u32 + 1;
            (
                w.elements,
                Vec::new(),
                1,
...
mod tests {
    ...
    // ---------------------------------------------------------- synth room
    //
    // The second sample world. `demo_room_circuit` and every test that names
    // it are untouched; everything below is about `EE_WORLD=synth`.

    /// `EE_WORLD` unset must still be the showcase, byte for byte.
    #[test]
    fn a_fresh_room_still_boots_the_showcase_by_default() {
        assert!(
            std::env::var("EE_WORLD").is_err(),
            "the test binary must not have EE_WORLD set"
        );
        let w = world();
        assert_eq!(w.hoist_rect, HOIST_RECT);
        assert!(w.panels.is_empty());
        let demo = demo_room_circuit();
        assert_eq!(w.elements.len(), demo.len());
        for (a, b) in w.elements.iter().zip(demo.iter()) {
            assert_eq!(a.id, b.id);
