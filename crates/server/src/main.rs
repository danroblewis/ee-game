...
fn checkpoint(room: &Room, hoist: &Hoist, damage: &DamageModel) {
    let save = SaveFile {
        hoist: *hoist,
        damage: damage.clone(),
...
async fn sim_task(
    ...
    loop {
        ...
        if room.events.receiver_count() > 0 {
            ...
            let _ = room
                .events
                .send(machine_msg(&hoist, motor_i, impact, motor_i_max()).to_string());
        }
...
/// a landing happened. `imax` is the motor's nameplate current from the
/// damage table — the client engraves it on the faceplate rather than
/// hardcoding a number that could drift from the model that enforces it.
fn machine_msg(hoist: &Hoist, motor_i: f64, impact: f64, i_max: f64) -> serde_json::Value {
    json!({
        "imax": i_max,
        "t": "machine",
        "id": MOTOR_ID,
...
async fn main() {
    ...
    let saved = load_room();
    let (mut elements, probes, next_pid, panels, next_plid, hoist, damage) = match saved {
        Some(s) => {
            tracing::info!(
    ...
    tokio::spawn(sim_task(room.clone(), cmd_rx, hoist, damage));
...
mod tests {
    ...
    fn machine_protocol_matches_the_contract() {
        ...
        hoist.landings = 3;
        hoist.joules = 12.5;
        let v = machine_msg(&hoist, 0.94, 1.75, motor_i_max());
