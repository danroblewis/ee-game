...
import { probeColor, renderScope, type Probe, type TraceStore } from './scope';
...
export interface Dock {
  ...
  update(now: number, probes: Probe[], traces: TraceStore, timebase: number): void;
}
...
export function createDock(root: HTMLElement, cv: HTMLCanvasElement): Dock {
  ...
  function update(now: number, probes: Probe[], traces: TraceStore, timebase: number) {
    if (probes.length === 0) {
      root.style.display = 'none';
      return;
    }
    root.style.display = 'block';
    if (open) renderScope(cv, traces, probes, timebase);
