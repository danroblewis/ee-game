...
export function createHoist(root: HTMLElement, opts: { reset: () => void }): Hoist {
  ...
  function drawPlate(
    ...
    // The rating line is second, and deliberately loud: it is the difference
    // between a working hoist and a dead motor. `imax` comes from the
    // server's damage table, so the plate can never promise a limit the
    // model does not enforce.
    const lines: [string, string][] = [
      ['#8ea3b1', 'M+  M−   |   SENSE A W B   |   LIM-TOP   |   LIM-BOT'],
      ['#e8a04a', `MOTOR ${ratedA(mm)} MAX — STALLED ROTOR DRAWS V/R. CONTROL IT.`],
